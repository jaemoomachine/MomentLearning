...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.series[idx: idx + self.seq_len]             # (seq_len, features)
        y = self.series[idx + self.seq_len: idx + self.seq_len + self.pred_len, self.tgt_idx]  # (pred_len,)
...
def extract_gamma_matrix(moment_ffn: MomentFFN):
    """
    MomentFFN 내부에 있는 MomentLinear 계층들의 gamma 파라미터를
    (L_sub × K) 모양의 행렬로 모아 반환합니다.
    """
    gammas = []
    for layer in moment_ffn.layers:
        if isinstance(layer, MomentLinear):
            # layer.gamma: 길이 (K+1) tensor, [γ0, γ1, γ2, ..., γK]
            # 여기서는 γ1,…,γK 만 사용
            gammas.append(layer.gamma[1:].unsqueeze(0))  # shape (1, K)
    # L_sub × K 행렬
    return torch.cat(gammas, dim=0)  # (L_sub, K)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x, y in train_loader:
            ...
            if args.regularizer != 'no' and isinstance(model, MoME):
                # MoME.experts: ModuleList of MomentFFN
                gamma_matrices = [extract_gamma_matrix(exp) for exp in model.experts]
            elif args.regularizer != 'no' and isinstance(model, HMoME):
                # HMoME는 트리 구조이므로, 가장 하위 레벨의 모든 MomentFFN을 모아야 함
                def collect_moment_ffns(node):
                    if isinstance(node, MomentFFN):
                        return [node]
                    ffns = []
                    for child in getattr(node, 'branches', []):
                        ffns += collect_moment_ffns(child)
                    return ffns

                all_ffns = collect_moment_ffns(model.root)
                # 전문가 수 M^depth 행렬 리스트
                gamma_matrices = [extract_gamma_matrix(ffn) for ffn in all_ffns]
            else:
                gamma_matrices = []
